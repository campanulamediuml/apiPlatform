...
#检索最近更改订单
    def recently_changed_orders(execute_command):
        ...
        access_token =wish_common.get_wish_access(execute_command['store_id'])
        url = "https://merchant.wish.com/api/v2/order/multi-get?start=%s&limit=%s&since=%s&access_token=%s"%(start,limit,since,access_token)
        r = requests.get(url)
        result = r.text
        # print(r.text)
        # result = common_unit.xmltojson(r.text)
...
#履行订单
    def fulfill_orders(execute_command):
        ...
        access_token =wish_common.get_wish_access(execute_command['store_id'])
        url = "https://merchant.wish.com/api/v2/order/fulfill-one?access_token=%s&tracking_provider=%s&tracking_number=%s&id=%s"%(access_token,tracking_provider,tracking_number,order_id)
        r = requests.post(url)
        # print(r.text)
        result = r.text
        # result = common_unit.xmltojson(r.text)
...
#退款/取消订单
    def refund_order(execute_command):
        ...
        access_token =wish_common.get_wish_access(execute_command['store_id'])
        url = "https://merchant.wish.com/api/v2/order/refund?access_token=%s&reason_code=%s&id=%s" % (access_token,reason_code,order_id)
        r = requests.post(url)
        # print(r.text)
        result = r.text
        # result = common_unit.xmltojson(r.text)
...
#在装运前修改订单的发货地址
    def modify_address_order(execute_command):
        ...
        access_token =wish_common.get_wish_access(execute_command['store_id'])
        order_id = quote(execute_command['order_id'])
        url = "https://merchant.wish.com/api/v2/order/change-shipping?access_token=%s&street_address1=%s&city=%s&state=%s&country=%s&id=%s"%(access_token,street_address1,city,state,country,order_id)
        r = requests.post(url)
        # print(r.text)
        result = r.text
        # result = common_unit.xmltojson(r.text)
...
#取消批量订购下载
    def batch_order_download_cancel(execute_command):
        job_id = quote(execute_command['job_id'])
        url = "https://merchant.wish.com/api/v2/order/cancel-download-job?job_id=%s"%(job_id)
        r = requests.post(url)
        # print(r.text)
        result = r.text
        # result = common_unit.xmltojson(r.text)
